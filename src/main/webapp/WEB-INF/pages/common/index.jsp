...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btnEp").click(function(){
			...
			if($.trim(newPassword) == '') {
				$.messager.alert("Error","password can't be null","error");
				return;
			}
			
			if(newPassword != $.trim($("#txtRePass").val())) {
				$.messager.alert("Error","password should be the same","error");
				return;
			}
			
			$.post("${pageContext.request.contextPath}/user_editpassword.action",{password:newPassword},function(data) {
	...
	function logoutFun() {
		...
		.confirm('系统提示','您确定要退出本次登录吗?',function(isConfirm) {
			if (isConfirm) {
				location.href = '${pageContext.request.contextPath }/exit.jsp';
			}
		});
	}
	// 修改密码
